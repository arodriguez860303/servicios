...
function send_tramite(idFormulario)
{
    ...
    $.ajax({
        type: 'POST',
        url: vuri + '/mis-tramites-hospital',
        data: vformData,
        processData: false,
        contentType: false,
        headers: {
        ...
        beforeSend: function() {
            $(".btn-enviar-tramite").prop('disabled', true);
        },
        success: function(vresponse, vtextStatus, vjqXHR) {
            ...
            else
            {
                if ( vresponse.codigo == 0 )
                        $(".btn-enviar-tramite").prop('disabled', false);
...
{ 
    $.ajax({
        type: 'POST',
        url: vuri + '/mis-tramites-hospital/solicitud/cancelar',        
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        beforeSend: function() {
