...
$(document).ready(function(e){
    $('#id_tipo_tramite').on('change', function() {
        if(this.value == 2)
        {
            $('.dvHospital').show();
            getHospitales();
...
function getMunicipios(selectOpt=0)
{
    $.ajax({
        ...
        success:  function (json) { 
            ...
            var string='';
            string+= '<option value=0>Seleccionar municipio...</option>';
...
function getTipoTramite(selectOpt=0)
{
    $.ajax({
        type: "GET",
        url:   vuri + '/tipo/get-tipo-tramite',
        dataType: "JSON",
        data:{
            method: 'get'                   
        },                
        success:  function (json) { 
            ...
            var string='';
            string+= '<option value=0>Seleccionar tipo de tramite...</option>';
            for(var i=0; i<json.respuesta.length; i++)
            {
                string+= '<option value='+json.respuesta[i].id+'>'+ json.respuesta[i].clave +' - '+ json.respuesta[i].nombre+'</option>';              
            }                               
            $('#id_tipo_tramite').html(string); 
            if (selectOpt != 0 ) $('#id_tipo_tramite').val(selectOpt).trigger('change');
        },
        error:function(){
        }
    });
