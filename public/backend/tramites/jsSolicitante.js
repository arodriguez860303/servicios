...
function loadDatosSolicitante()
 {
    ...
    $.ajax({
        type: 'GET',
        url: vuri + '/tramites/datos-solicitante',
        dataType: "JSON",
        data: {
            method: 'show',
        ...
        success: function(vresponse, vtextStatus, vjqXHR) {
            ...
            if ( parseInt(vresponse.codigo) == 1 ) {
                ...
                $('#spFolioPago').html(vrespuesta.folio_hacienda);
                ...
                $('#spTelefono').html(vrespuesta.telefono);
                $('#spDomicilio').html(vrespuesta.calle +' entre '+ vrespuesta.entre_calle +' y '+ vrespuesta.y_calle +', #'+ vrespuesta.num_exterior);
                $('#spCP').html(vrespuesta.codigo_postal);
                $('#spMunicipio').html(vrespuesta.municipio + ', Chiapas.');
            }
        },
...
{    
    $.ajax({
        type: 'GET',
        url: vuri + '/tramites/datos-generales-tramite',
        dataType: "JSON",
        data: {
            method: 'show',
            id_tramite: $('#id_tramite').val()
        ...
        success: function(vresponse, vtextStatus, vjqXHR) {
            ...
            $('#txtFolioPago').html(vrespuesta.folio_hacienda); 
            $('#txtRazonSocial').html(vrespuesta.razon_social_o_nombre); 
            $('#txtRFC').html(vrespuesta.rfc); 
            $('#txtCorreo').html(vrespuesta.correo); 
            $('#txtTelefono').html(vrespuesta.telefono); 
            $('#txtDireccion').html(vrespuesta.calle +' entre '+ vrespuesta.entre_calle +' y '+ vrespuesta.y_calle +' No '+ vrespuesta.num_exterior +', Colonia '+ vrespuesta.colonia +', CP '+ vrespuesta.codigo_postal); 
            $('#txtMunicipio').html(vrespuesta.municipio);             
...
function datos_documentos()
{
    $.ajax({
        type: 'GET',
        url: vuri + '/tramites/datos-documentacion',
        dataType: "JSON",
        data: {
            method: 'get',
            id_tramite: $('#id_tramite').val()
        ...
        success: function(vresponse, vtextStatus, vjqXHR) {
            ...
            if ( parseInt(vresponse.codigo) == 1 ) {
                ...
                for ( vi=0; vi < vrespuesta.length; vi++ ) {
                    ...
                    let download = vuri + "/tramites/datos-documentacion/" + vrespuesta[vi].id_documento_tramite + "/descargar";
                ...
                // if(observados > 0){
                //     $('#btn-autorizar-tramite').hide();

                //     if(globalStatus != 8 && globalStatus != 5)
                //         $('#btn-observar-tramite').show();
                // }              
...
function datos_evaluacion()
{
     $.ajax({
        type: 'GET',
        url: vuri + '/tramites/resultados-evaluacion',
        dataType: "JSON",
        data: {
            method: 'show',
            id_tramite: $('#id_tramite').val()
...
function store_observacion()
 {
    ...
    if ( validaForm('frmObservacion') ) {
        $.ajax({
            type: "POST",
            url: vuri + '/tramites/datos-documentacion/observar',
            data: vformData,
            processData: false,
            contentType: false,
            headers: {
...
function load_observaciones()
{
    ...
    $.ajax({
        type: 'GET',
        url: vuri + '/tramites/datos-observaciones',
        dataType: "JSON",
        data: {
            method: 'get',
            id_tramite: $('#id_tramite').val()
...
function send_observaciones()
{
    $.ajax({
        type: 'POST',
        url: vuri + '/tramites/'+ $('#id_tramite').val() +'/enviar-observaciones',      
        processData: false,
        contentType: false,
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
...
function documento_valido(id_documento_tramite)
{
    $.ajax({
        type: 'POST',
        url: vuri + '/tramites/datos-documentacion/aceptar',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        data: {
...
function loadSolventaciones()
{
    $.ajax({
        type: 'GET',
        url: vuri + '/tramites/datos-solventaciones',
        dataType: "JSON",
        data: {
            method: 'get',
            id_tramite: $('#id_tramite').val()
        ...
        success: function(vresponse, vtextStatus, vjqXHR) {
            ...
            if ( parseInt(vresponse.codigo) == 1 ) {
                ...
                for ( vi=0; vi < vrespuesta.length; vi++ ) {  
                    ...
                    vhtml+='              <i class="fas fa-check-circle fa-lg"></i>';
                    vhtml+='            </a>';
                    ...
                    vhtml+='              <i class="fas fa-times-circle fa-lg"></i>';
                    vhtml+='            </a>';
                    vhtml+='          </div>';
                    }
                    else if(vrespuesta[vi].aceptado == 10){
                        vhtml+='<i class="far fa-check-circle fa-lg text-success"></i> Aceptado';
                        aceptados++;
                    }
                    else if(vrespuesta[vi].aceptado == 4)
                        vhtml+='<i class="far fa-times-circle fa-lg text-danger"></i> Rechazado';
                    vhtml+='        </td>';                    
                    vhtml+='      </tr>  ';
                }
                vhtml+='    </tbody>';
...
function solventacion_valida(id_observacion)
{
    $.ajax({
        type: 'POST',
        url: vuri + '/tramites/datos-solventaciones/aceptar',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        data: {
...
function solventacion_negada(id_observacion)
{
    $.ajax({
        type: 'POST',
        url: vuri + '/tramites/datos-solventaciones/negar',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        data: {
