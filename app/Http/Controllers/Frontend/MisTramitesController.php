...
class MisTramitesController extends Controller
{
    ...
    public function header() 
    {
        ...
        try {
                ...
                $vrespuesta['encabezado']=T_Registro::dtsGenerales($vfiltro)->first();
        }
    ...
    public function datos_generales() 
    {
        ...
        try {
                $vfiltro=array();  
                $vfiltro['id_registro'] =  Auth::User()->id_registro;
                $vrespuesta['datos']=T_Registro::dtsRegistro($vfiltro)->first();
        }
        catch(Exception $vexception ){
            $vstatus=500;
            $vrespuesta['message']=$vexception->getMessage();
    ...
    public function getTramites(Request $vrequest)
    {
        ...
        try {
            ...
            switch ($vrequest->method) {
                case 'show':
                    $vresponse['respuesta']=T_Tramite::queryToDB(['id'=>$vrequest->id])->first();
                  break; 
                case 'get':
                    $vresponse['respuesta']=T_Tramite::queryToDB(['id_registro' =>Auth::User()->id_registro])->get();
                  break;               
                default:
    ...
    {    
        $datos_registro = T_Registro::find(Auth::User()->id_registro);
    	return view('frontend.mis-tramites.index');
    }
    ...
    public function create() 
    {
        ...
        $vfiltro=array();
        $vflPreguntas=T_Preguntas::lstPreguntas($vfiltro)->get();        

        return view('frontend.mis-tramites.create', ['id_registro'=>$id_registro, 'preguntas'=>$vflPreguntas]);
    }
    ...
    public function store(Request $vrequest)
    {
        ...
        $post_tramite['id_tipo_tramite']=1;
        ...
        $turnar_status= $turnar->asignar_responsables_alimentos();
        if($turnar_status==true) {
            $array= [];                                 
            $post_tramite['id_usuario_r']= $turnar->getResponsableAlimentos();
        }

        //Se verifica que el folio de hacienda no se encuentre registrado
        $verifica_folio = T_Tramite_Constancia::existePago(['folio_hacienda'=>$t_constancia->folio_hacienda, 'id_registro'=>$id_registro]); 

        if(isset($verifica_folio->folio_hacienda))
        {
            $vresponse=['icono'=>'warning', 'codigo'=> 0, 'mensaje'=> 'El folio de pago ya se encuentra registrado.', 'url'=> '', 'data'=>''];
        }
        else
        {
            try {
                    DB::beginTransaction();
                        ...
                        foreach ($t_documentos as $value) {
                            ...
                            $vrutaCarpeta='/expedientes/'. $anio_tramite .'/alimentos/'. $registro->curp .'/exp_'. $t_tramite->id;
    ...
    public function datos_folio_hacienda() 
    {
        $vstatus=200;
        $vrespuesta=array();
        try {
    ...
    public function store_folio_pago(ConstanciaRequest $request)
    {
        $status               = 1;
        $code                 = 201;
        $post                 = $request->all();
        ...
        if(isset($constancia->folio_hacienda))        
            $t_tramite_constancia = T_Tramite_Constancia::find($constancia->id);        
        else        
            $t_tramite_constancia = new T_Tramite_Constancia;        

        $post_folio["id_registro_temp"]    = Auth::User()->id_registro;
        $post_folio["folio_hacienda"]      = $post['txtFolioPago'];
        $post_folio["id_usuario_registro"] = Auth::User()->id_registro;

        //Se verifica que el folio de hacienda no se encuentre registrado
        $verifica_folio = T_Tramite_Constancia::existePago(['folio_hacienda'=>$post['txtFolioPago']]); 

        if(isset($verifica_folio->folio_hacienda))
        {            
            $status = 3;
            $code = 409;
            $msg = 'El folio de pago ya se encuentra registrado.';
            $data='';
        }
        else
        {
            try {
                    ...
                    $msg                      = "El folio de pago ha sido guardado satisfactoriamente";
                    $data                     = $t_tramite_constancia;
    ...
    public function detalle($id){     
        ...
        $datos = T_Tramite::queryToDB(['id_tramite'=>$id])->first();
        return view('frontend.mis-tramites.show', ['datos'=>$datos, 'id'=>$id]);
    }
}