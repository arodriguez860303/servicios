...
{   
    public function lstObservaciones(Request $vrequest)
    {
        $vHTTPCode=200;
    ...
    {    
    	return view('frontend.observaciones.index', ['id_tramite'=>$id_tramite]);
    } 
    ...
    public function solventar(SolventarRequest $vrequest)
    {
        ...
        $t_tramite  = T_Tramite::find($id_tramite);

        $anio_tramite= FormatDate::anio($t_tramite->created_at,1);

        $folder     = '/expedientes/' . (int)$anio_tramite . '/alimentos/' . $t_registro->curp . '/exp_'.$id_tramite.'/solventacion';
    ...
    public function enviar_solventaciones(Request $vrequest)    
    {
        ...
        try {
                DB::beginTransaction();
                    ...
                    $vresponse=['icono'=>'success', 'codigo'=> 1, 'mensaje'=> 'Las solventaciones fueron enviadas satisfactoriamente.','url'=> route('mis-tramites.index')];
